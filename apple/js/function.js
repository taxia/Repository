...
* @Last Modified time: 2017-08-16 16:58:54
*/
...
HTMLElement.prototype.next = function(){
	let next = this.nextElementSibling;
	let back = next?next:null;
	return back;
}

//下面全部元素
HTMLElement.prototype.nextAll = function(){
	let nextAll = this.next();
	let newArr = [];
	while(nextAll){
		newArr.push(nextAll);
		nextAll = nextAll.next();
	}
	return newArr;
}
...
HTMLElement.prototype.previous = function(){
	...
	let back = previous?previous:null;
	return back;
}

//上面全部元素
HTMLElement.prototype.previousAll = function(){
	let previousAll = this.previous();
	let newArr = [];
	while(previousAll){
		newArr.push(previousAll);
		previousAll = previousAll.previous();
	}
	return newArr;
}
